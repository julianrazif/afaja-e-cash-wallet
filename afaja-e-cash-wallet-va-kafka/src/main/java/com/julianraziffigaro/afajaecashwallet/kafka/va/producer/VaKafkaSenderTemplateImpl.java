package com.julianraziffigaro.afajaecashwallet.kafka.va.producer;
...
@Service
public class VaKafkaSenderTemplateImpl implements KafkaSenderTemplate {
  ...
  private Flux<SenderRecord<Long, String, Long>> createOutBoundFlux(KafkaMessage messageValue) {
    return Flux.just(messageValue)
      .map(i -> {
        ...
        log.info("Sent Data --> {}", kafkaRecord.value());
        return kafkaRecord;
      });
  }
}
